/* ============================================================================
 * ui.js — shared UI helpers for the scoreboard, delay-feed + game pages
 * (DOM builders, team colors, count dots, runners diamond, status chips,
 *  forecast-risk chips, irregularity flag lists, official source-link rows)
 * ==========================================================================*/
'use strict';
...
const UI = (() => {
  ...
  /** Forecast-risk chip: level ∈ high|moderate|low|none|covered|unknown. */
  function riskChip(level, labelOverride, title) {
    const meta = (typeof Weather !== 'undefined' && Weather.RISK_META && Weather.RISK_META[level]) ||
      { label: level || '—', cls: 'risk-unknown', glyph: '' };
    const chip = el('span', `chip ${meta.cls}`, `${meta.glyph ? `${meta.glyph} ` : ''}${labelOverride || meta.label}`);
    if (title) chip.title = title;
    return chip;
  }

  /** Delay-kind chip from Delays.KIND_META (falls back to a muted chip). */
  function kindChip(kind, labelOverride) {
    const meta = (typeof Delays !== 'undefined' && Delays.KIND_META && Delays.KIND_META[kind]) ||
      { label: labelOverride || kind || '—', cls: 'chip-muted', glyph: '' };
    const chip = el('span', `chip ${meta.cls}`, `${meta.glyph ? `${meta.glyph} ` : ''}${labelOverride || meta.label}`);
    if (meta.cls === 'chip-live' || meta.cls === 'chip-warn' || meta.cls === 'chip-review') chip.prepend(el('span', 'chip-dot'));
    return chip;
  }

  /**
   * Irregularity flags box. Each flag is { code, text }; the code is shown
   * verbatim so a reviewer can grep for the rule that raised it.
   */
  function flagList(flags, heading) {
    const list = (flags || []).filter(Boolean);
    if (!list.length) return null;
    const box = el('div', 'flag-list');
    box.appendChild(el('div', 'flag-list-head', heading || `⚠ ${list.length} irregularit${list.length === 1 ? 'y' : 'ies'} flagged for manual review`));
    list.forEach((f) => {
      const row = el('div', 'flag-item');
      if (f.code) row.appendChild(el('code', null, f.code));
      row.appendChild(document.createTextNode(`${f.code ? ' ' : ''}${f.text || ''}`));
      box.appendChild(row);
    });
    return box;
  }
  ...
  /**
   * Row of official source links: [{ label, url, kind? }] where kind ∈
   * 'mlb' (default) | 'gov' | 'model'. Every rendered fact should be
   * traceable to at least one of these.
   */
  function sourceLinks(links, label) {
    const list = (links || []).filter((l) => l && l.url);
    if (!list.length) return null;
    const row = el('div', 'source-links');
    row.appendChild(el('span', 'source-links-label', label || 'Verify'));
    list.forEach((l) => {
      const a = el('a', `source-link ${l.kind === 'gov' ? 'source-link-gov' : l.kind === 'model' ? 'source-link-model' : ''}`, l.label || l.url, {
